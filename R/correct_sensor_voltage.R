    ...
    if (sign == 1) # for PR2 and ThetaProbe: raw value (voltages) increases with permittivity
      {
        ...
        if (beyond_air) {
          warn_str <- paste0(
            "Min ", unique_settings$var_type[ss], " in time series (", Vmin_measured, unit, ") smaller than reference ",
          ...
          if (adjust_range) {
            warn_str <- paste0(
              probe_id_str, ": Min ", unique_settings$var_type[ss], " in time series (", Vmin_measured, unit, ") smaller than reference ",
        ...
        if (beyond_water) {
          warn_str <- paste0(
            "Max ", unique_settings$var_type[ss], " in time series (", Vmax_measured, unit, ") larger than reference ",
            unique_settings$var_type[ss], " for water (", unique_settings[ss, "var_h2o_meas"], unit,
            "). Consider updating calibration data for ", probe_id_str, "'.\n"
          )
          if (adjust_range) {
            warn_str <- paste0(
              probe_id_str, ": Max ", unique_settings$var_type[ss], " in time series (", Vmax_measured, unit, ") larger than reference ",
...
# function to calibrate also everything outside quantil interval
correct_sensor_values_all <- function(values, serial_no=NULL, probe_id=NULL, var_type=NULL, ring_no=1, calib_data, warnOnly=FALSE, adjust_range=TRUE, ...)
# converts sensor values of var_type (Voltage [V], Permittivity [-], Counts [-] ) according to calibration data in calib_data for the specified sensor and ring
{
  # tt = get_reference_voltage(serial_no = serial_no, probe_id = probe_id, ring_no = ring_no, calib_data = calib_data)
  ...
  for (ss in 1:nrow(unique_settings))
  {
    ...
    Vmax_measured <- values_range_measured[2] # get maximum voltage measured, discarding outliers
    Vmin_measured <- values_range_measured[1] # get minimum voltage measured, discarding outliers
    ...
    if (!adjust_range) {
      values_adj <- values
      # values_adj[(values < min(unique_settings[ss, c("var_air_meas", "var_h2o_meas")])) |
      #        (values > max(unique_settings[ss, c("var_air_meas", "var_h2o_meas")])) ] = NA

      # use adjusted for corrections
      values_corrected[cur_rows] <- var_corr(
        x = values_adj[cur_rows],
        var_air_meas = unique_settings[ss, "var_air_meas"],
        var_h2o_meas = unique_settings[ss, "var_h2o_meas"],
        type = unique_settings[ss, "type"],
        var_type = unique_settings[ss, "var_type"],
